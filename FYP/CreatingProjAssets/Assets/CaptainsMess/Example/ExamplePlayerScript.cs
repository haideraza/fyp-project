...
public class ExamplePlayerScript : CaptainsMessPlayer
{
	...
	void OnGUI()
	{
		if (isLocalPlayer)
		{
			...
			if (gameSession)
			{
				if (gameSession.gameState == NetGameState.Lobby ||
					gameSession.gameState == NetGameState.Countdown)
				{
				...
				else if (gameSession.gameState == NetGameState.WaitingForRolls)
				{
					if (rollResult == 0)
					{
				...
				else if (gameSession.gameState == NetGameState.GameOver)
				{
					if (isServer)
					{
